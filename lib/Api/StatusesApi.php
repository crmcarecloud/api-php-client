...
class StatusesApi
{
     ...
     * Get a status
     *
     * @param  string $status_id The unique ID of the status. (required)
    ...
    {
        list($response) = $this->getStatusWithHttpInfo($status_id, $accept_language);
        return $response;
    }
     ...
     */
    public function getStatusAsync($status_id, $accept_language = 'cs, en-gb;q=0.8')
    {
        return $this->getStatusAsyncWithHttpInfo($status_id, $accept_language)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     */
    protected function getStatusRequest($status_id, $accept_language = 'cs, en-gb;q=0.8')
    {
        // verify the required parameter 'status_id' is set
        if ($status_id === null || (is_array($status_id) && count($status_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $status_id when calling getStatus'
            );
        }

        $resourcePath = '/statuses/{status_id}';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
